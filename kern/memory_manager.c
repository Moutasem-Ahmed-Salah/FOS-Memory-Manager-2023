...
{
		int n=ROUNDUP(size, PAGE_SIZE)/PAGE_SIZE;
		for(int i=0; i<n; i++) {
...
void __freeMem_with_buffering(struct Env* e, uint32 virtual_address, uint32 size)
{
		...
		{
		    uint32 pagePermissions = pt_get_page_permissions(e, VirtualAddress);
			if(pagePermissions & PERM_BUFFERED)
			{
				pageTable = NULL;
			    struct Frame_Info* frameInfo = get_frame_info(e->env_page_directory, (void*)VirtualAddress, &pageTable);

				if(pagePermissions & PERM_MODIFIED)
				{

					bufferlist_remove_page(&modified_frame_list,frameInfo);
					frameInfo ->isBuffered = 0;
					frameInfo ->environment = NULL;
					free_frame(frameInfo);
					pt_clear_page_table_entry(e,VirtualAddress);
				}
				else
				{
					frameInfo ->isBuffered = 0;
					frameInfo ->environment = NULL;
					bufferlist_remove_page(&free_frame_list, frameInfo);
					free_frame(frameInfo);
                    pt_clear_page_table_entry(e,VirtualAddress);
				}
		...
		{
			uint32 WSAddress = env_page_ws_get_virtual_address(e,i);
			if(WSAddress>= VirtualAddress && WSAddress < VirtualAddress + size)//within range betaa va
			{
				pageTable = NULL;
				struct Frame_Info* frameInfo = get_frame_info(e->env_page_directory, (void*)WSAddress, &pageTable);
				if(frameInfo != NULL)
				{
				      unmap_frame(e->env_page_directory,(void*)WSAddress);
					  env_page_ws_clear_entry(e,i);
					  pf_update_env_page(e, (void *)WSAddress, frameInfo);
				}
		...
		for(int i=0; i < NumberofPages; i++)
		{
			...
			pageTable = NULL;
			get_page_table(e->env_page_directory,(void*)VirtualAddress,&pageTable);
			if(pageTable != NULL)
			{
				int emptyPageTable = 1;
				...
				if(emptyPageTable)
				{
					kfree((void*)pageTable); //bafree el memory allocated for the page table.
					pd_clear_page_dir_entry(e,VirtualAddress);//page no longer exists
				}
			}
