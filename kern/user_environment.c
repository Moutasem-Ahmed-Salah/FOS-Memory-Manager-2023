...
void __env_free_with_buffering(struct Env *e)
{
    ...
    for(int WsIndex = 0;WsIndex  < e->page_WS_max_size; WsIndex++)
    {
    	uint32 vaddress = env_page_ws_get_virtual_address(e,WsIndex );
    	unmap_frame(e->env_page_directory,(void*)vaddress);
    	env_page_ws_clear_entry(e,WsIndex);
    	pf_update_env_page(e, (void *)vaddress, frame_info);
    }
	...
	// [4] Free all TABLES from the main memory
	 ...
	 for (uint32 pdindex = 0; pdindex < PDX(USER_TOP); pdindex++)
	    {
          uint32 pa=e->env_page_directory[pdindex];

         uint32 VA= kheap_virtual_address(pa);

	        if (e->env_page_directory[pdindex] & PERM_PRESENT)
	        {
